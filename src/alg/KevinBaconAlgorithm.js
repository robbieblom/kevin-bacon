...
export class KevinBaconAlgorithm {
  ...
  async run() {
    const connectionPath = await this.isConnectedToActor(
      this.source_actor,
      this.target_actor
    );
    const formattedConnectionPath = await this.formatConnectionPath(
      connectionPath
    );
  ...
  async formatConnectionPath(connectionPath) {
    const formatted = Promise.all(
      connectionPath.map(async (node, index) => {
        if (index % 2 == 0) {
