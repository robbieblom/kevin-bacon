...
export const ActorSelector = (props) => {
  ...
  const fetchActors = useMemo(
    () =>
      debounce(async (actor_name, callback) => {
        MovieService.getActorByName(actor_name).then(callback);
      }, 400),
    []
  );
  ...
  const startAdornment = selectedActor ? (
    <Box sx={{ paddingRight: "10px" }}>
      <Avatar
        src={MovieService.getPosterImageURL(selectedActor.profile_path)}
      />
    </Box>
  ) : null;
            ...
            <li {...props}>
              <Grid container alignItems="center">
                <Grid item sx={{ display: "flex", width: 54 }}>
                  <Avatar
                    src={MovieService.getPosterImageURL(option.profile_path)}
                  />
                </Grid>
                <Grid
                  item
